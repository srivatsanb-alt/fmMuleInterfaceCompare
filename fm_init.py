...
fleet_sherpas = toml.load(os.path.join(os.getenv("FM_CONFIG_DIR"), "config.toml"))["fleet_sherpas"]
for sherpa_name, sherpa_detail in fleet_sherpas.items():
    fu.add_sherpa(sherpa_name=sherpa_name,
                  hwid=sherpa_detail["hwid"],
                  api_key=sherpa_detail["api_key"],
                  fleet_id=1
                  )
    fu.add_sherpa_to_fleet(sherpa=sherpa_name,
                           fleet=sherpa_detail["fleet_name"]
                           )
