...
download_pull_fm_update()
{
  redis-cli -p $REDIS_PORT set update_done false
   ...
   curl -X "GET" $HTTP_SCHEME://$STATIC_FILE_AUTH_USERNAME:$PASSWORD@$MASTER_FM_IP:$MASTER_FM_PORT/api/static/downloads/$PROD/fm/$FM_VERSION/docker_compose_$FM_VERSION.yml > /app/static/docker_compose_$FM_VERSION.yml
