...
@router.post("/{entity_name}/disable/")
async def disable_station(entity_name: str, user_name=Depends(dpd.get_user_from_header)):
    response = {}
    ...
    with DBSession() as dbsession:
        ...
        for ongoing_trip in all_ongoing_trips:
            ...
            if entity_name in stations:
                raise ValueError(f"can not disable station :{entity_name} ,ongoing trip : {trip.id} with station : {entity_name} in trip route : {trip.route}")
    

        station_status: fm.StationStatus = dbsession.get_station_status(entity_name)
        station_status.disabled = True
