...
async def update_ws(
    ...
    logger.info(
        f"websocket connection(generic updates) established successfully with client {x_real_ip}"
    )
    ...
    while True:
        ...
        if message:
            ...
            try:
                ...
                logger.info(
                    f"websocket connection(generic updates) disconnected with client {x_real_ip} "
                )
                raise e
